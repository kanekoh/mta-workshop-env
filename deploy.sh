...
EOF
    ...
    # GPU ノードプールのプリセット（GPU_MACHINE_TYPE が設定されている場合に自動生成）
    if [ -n "${GPU_MACHINE_TYPE:-}" ] && [ "${GPU_REPLICAS:-0}" != "0" ]; then
        local gpu_replicas="${GPU_REPLICAS:-1}"
        log_info "GPU ノードプール設定を gpu-pool.auto.tfvars に書き出し中..."
        log_info "  マシンタイプ: ${GPU_MACHINE_TYPE}, レプリカ: ${gpu_replicas}"
        cat > gpu-pool.auto.tfvars << EOF
# GPU node pool (automatically set from profile GPU_MACHINE_TYPE / GPU_REPLICAS)
# This file is automatically generated by deploy.sh
# Do not edit this file manually - it will be overwritten on each deployment
...
EOF
        ...
        # GPU 不要の場合はファイルを削除（前回の残りを除去）
        rm -f gpu-pool.auto.tfvars
    fi

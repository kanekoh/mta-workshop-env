#!/bin/bash
# Create S3 bucket for OpenShift Loki (Network Observability)
...
# S3バケット名（Loki 専用）
BUCKET_NAME="${CLUSTER_NAME}-loki-${AWS_ACCOUNT_ID}"

echo "Creating S3 bucket for Loki: ${BUCKET_NAME}"
echo "  Region: ${REGION}"
...
echo ""
echo "S3 bucket for Loki setup completed!"
